...
class SpeechToTextTranscriber:
    def __init__(self):
        ...
        self.model_id_en = "facebook/wav2vec2-base-960h"
        self.model_id_multi = "facebook/wav2vec2-large-xlsr-53"  # Multilingual model
        self.model = Wav2Vec2ForCTC.from_pretrained(self.model_id_en).to(self.device)  # Default to English
        self.processor = Wav2Vec2Processor.from_pretrained(self.model_id_en)

        self.audio_language_model = EncoderClassifier.from_hparams(
    ...
    def _transcribe_file(self, file_path):
        ...
        try:
            ...
            # Select model based on language detection
            if audio_language in ["en", "eng"]:  # Example for English
                logging.info("Detected language is English, using English model.")
                self.model = Wav2Vec2ForCTC.from_pretrained(self.model_id_en).to(self.device)
                self.processor = Wav2Vec2Processor.from_pretrained(self.model_id_en)
            else:
                logging.info("Detected non-English language, switching to multilingual model.")
                self.model = Wav2Vec2ForCTC.from_pretrained(self.model_id_multi).to(self.device)
                self.processor = Wav2Vec2Processor.from_pretrained(self.model_id_multi)
