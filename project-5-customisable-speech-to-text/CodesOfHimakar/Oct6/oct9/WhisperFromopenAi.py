...
class AudioCapture:
    ...
    def finish_transcriptions(self):
        ...
        self.delete_files()  # Automatically delete files after transcription
...
class SpeechToTextTranscriber:
    def __init__(self):
        ...
        self.model_id = whisper.load_model('turbo') 
    ...
    def transcribe_real_time(self, audio_input):
        ...
        try:
            # Detect language from the audio using Whisper Turbo
            detected_language_list_tiny = self.detect_language_from_whisper_turbo(audio_input)
            logging.info(f"Detected Language by Turbo: {detected_language_list_tiny}")
            ...
            transcription = self.model_id.transcribe(audio_input, language = whisper_forced_language)
        ...
        if whisper_forced_language != "en":
            ...
            romanized_transcription = self.romanize_text(transcription['text']) 
            logging.info(f"Romanized transcription in English: {romanized_transcription}") 

        logging.info("*********************** End of Transcribing file **********************")

    def combine_lid_results(self, detect_language_from_whisper_turbo, detected_language_list_mms, detected_language_list_vox):
        """Combines language identification results from different models."""

        language_weights = {}

        # --- Whisper Tiny ---
        for language, prob in detect_language_from_whisper_turbo:
    ...
    def detect_language_from_whisper_turbo(self, audio_input, top_k = 5):
        """Detects language from the provided audio input using VoxLingua."""
        try:
            # Use Whisper Turbo to detect the language and get the top 5 probabilities
            audio = whisper.pad_or_trim(audio_input)  # Prepare the audio for Whisper Turbo
            mel = whisper.log_mel_spectrogram(audio, n_mels=128).to(self.model_id.device)  # Convert to Mel spectrogram

            # Get language probabilities from the Whisper Turbo model
            _, probs = self.model_id.detect_language(mel)
            ...
            logging.info("Top 5 detected languages by Whisper Turbo:")
            return sorted_probs
        except Exception as e:
            logging.error(f"Error detecting language with Turbo: {e}")
            return None
