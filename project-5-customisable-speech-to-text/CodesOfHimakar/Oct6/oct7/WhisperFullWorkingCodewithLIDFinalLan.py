...
class SpeechToTextTranscriber:
    ...
    def transcribe_real_time(self, audio_input):
        ...
        try:
            ...
            input_features = self.processor(audio_input, sampling_rate=16000, language=overall_language, return_tensors="pt").input_features.to(self.device)
            predicted_ids = self.model.generate(input_features, output_scores=True, return_dict_in_generate=True)
    ...
    def combine_lid_results(self, detected_language_whisper, detected_language_list_mms, detected_language_list_vox):
        ...
        final_language_ = max(language_weights, key=language_weights.get)  # Get key with max value
        final_language = round(language_weights[final_language_], 4)  # Round to 4 decimal places
        ...
        overall_language_ = max(self.overall_language_weights, key=self.overall_language_weights.get)
        overall_language = round(self.overall_language_weights[overall_language_], 4)  # Round to 4 decimal places
