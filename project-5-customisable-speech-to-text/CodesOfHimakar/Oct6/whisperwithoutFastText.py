...
class SpeechToTextTranscriber:
    ...
    def transcribe_real_time(self, audio_input):
        ...
        try:
            # Detect language using Whisper (top 5)
            whisper_detected_languages = self.detect_language_with_whisper(audio_input)
            logging.info(f"Direct audio language detection (Whisper): {whisper_detected_languages}")
            ...
            # Extract the predicted language token (it's usually the first token)
            predicted_language_token = predicted_ids.sequences[0, 1].item()  # Get the first token (language token)
            detected_language = self.processor.tokenizer.convert_ids_to_tokens([predicted_language_token])[0]

            # Map the language token to a human-readable language
            logging.info(f"Detected language token by Whisper: {detected_language}")
        ...
        logging.info(f"Direct audio language detection (MMS LID): {detected_language_list}")

        # Detect language from the audio using VoxLingua
        voxlingua_detected_language = self.detect_language_from_voxlingua(audio_input)
        logging.info(f"Direct audio language detection (VoxLingua): {voxlingua_detected_language}")
        ...
        if detected_language_list and detected_language_list[0] != "en":
        ...
        wh_tiny_ld = self.detect_language_with_tiny_whisper(audio_input)
        logging.info(f"Ld with tiny: {wh_tiny_ld}")

        tiny_list = self.print_top_languages(audio_input)
        logging.info(f"Ld with tiny: {tiny_list}")
    ...
    def detect_language_from_voxlingua(self, audio_input):
        ...
        try:
            ...
            predicted_language = language_prediction[3]
    ...
    def detect_language_from_audio(self, audio_input, top_k = 5):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                for i in range(top_k):
                    ...
                    #confidence_interval = (prob - probabilities.std().item(), prob + probabilities.std().item())  
                    #logging.info(f"Language: {lang}, Probability: {prob:.4f}, Confidence Interval: {confidence_interval}")
    ...
    def detect_language_with_whisper(self, audio_input, top_k = 5):
        """Detects language using Whisper and returns the top 10 probable languages."""
        try:
            input_features = self.processor(audio_input, sampling_rate=16000, return_tensors="pt").input_features.to(self.device)
            # Generate token ids with output_scores
            predicted_ids = self.model.generate(input_features, output_scores=True, return_dict_in_generate=True) 
            # Print the generated token IDs
            print("Generated token IDs:", predicted_ids.sequences) 

            # Get the scores for the first token (language token)
            language_token_scores = predicted_ids.scores[1][0]  # Scores for the first token in the first (and only) sequence
            probabilities = F.softmax(language_token_scores, dim=-1)  # Apply softmax to get probabilities

            # Get the indices of the top k probabilities
            top_k_probabilities, top_k_indices = torch.topk(probabilities, top_k)

            logging.info(f"Top {top_k} probable languages (Whisper):")
            top_languages = []
            for i in range(top_k):
                lang_token_id = top_k_indices[i].item()
                lang = self.processor.tokenizer.decode(lang_token_id)  # Decode the token ID to get the language
                prob = top_k_probabilities[i].item()
                logging.info(f"Language: {lang}, Probability: {prob:.4f}")
                top_languages.append((lang, prob))
    ...
    def detect_language_with_tiny_whisper(self, audio_input):
        audio_file = whisper.load_audio(audio_input)
        # Transcribe the audio file
        result = self.wl_model.transcribe(audio_file)
    
        # Extract and print the detected language
        detected_language = result['language']
        print(f"Detected Language for {audio_file}: {detected_language}")

    def print_top_languages(self, audio_file, top_k=5):  # Corrected function definition
        # Detect language probabilities
        audio = whisper.load_audio(audio_file)
        audio = whisper.pad_or_trim(audio)
        mel = whisper.log_mel_spectrogram(audio).to(self.device)  # Use self.device

        # Get language probabilities from the model
        _, probs = self.wl_model.detect_language(mel)  # Use self.wl_model

        # Sort the languages by probability and get the top k
        sorted_probs = sorted(probs.items(), key=lambda item: item[1], reverse=True)[:top_k]  # Use top_k

        print(f"Top {top_k} possible languages for {audio_file}:")
        for language, prob in sorted_probs:
            print(f"Language of tiny: {language}, Probability of tiny: {prob:.4f}")
