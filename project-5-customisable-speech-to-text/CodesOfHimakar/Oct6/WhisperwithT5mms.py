...
class SpeechToTextTranscriber:
    ...
    def transcribe_real_time(self, audio_input):
        ...
        try:
            ...
            detected_language_list = self.detect_language_from_audio(audio_input, top_k=5) 
...
# Unfortunately, SpeechBrain's EncoderClassifier for VoxLingua doesn't directly provide probabilities or a way to get the top k predictions. It only gives the single most likely language.
            ...
            whisper_detected_languages = self.detect_language_with_whisper(audio_input, top_k=5)
    ...
    def detect_language_with_whisper(self, audio_input, top_k = 5):
        ...
        try:
            input_features = self.processor(audio_input, sampling_rate=16000, return_tensors="pt").input_features.to(self.device)
            logits, probs = self.model.detect_language(input_features)

            top_languages = sorted(probs.items(), key=lambda item: item[1], reverse=True)[:top_k]
            logging.info("Top 10 probable languages (Whisper):")
