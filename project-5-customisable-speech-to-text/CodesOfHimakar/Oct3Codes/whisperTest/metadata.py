...
class AudioCapture:
    def __init__(self, sample_rate=16000, chunk_size=1024, silence_threshold=0.01, silence_duration=1):
        ...
        self.audio_processor = AudioProcessor(self.transcriber)

    def start_stream(self, callback):
        """Starts the audio stream with the given callback for processing."""
        self.audio_processor.clear_queue()  # Clear previous queue
    ...
    def stop_stream(self):
        ...
        if self.audio_stream:
            ...
            self.transcriber.wait_for_completion()  # Wait for all transcriptions to finish

class AudioProcessor:
    def __init__(self, transcriber, sample_rate=16000, chunk_size=1024, silence_duration=1, silence_threshold=0.01):
        self.sample_rate = sample_rate
        self.chunk_size = chunk_size
        self.silence_duration = silence_duration
        self.silence_threshold = silence_threshold
    ...
    def clear_queue(self):
        """Clears the audio queue at the start of a new stream."""
        while not self.audio_queue.empty():
            self.audio_queue.get()  # Discard the remaining audio data
    ...
    def process_audio(self, indata, frames, time, status):
        ...
        if len(self.current_audio) >= 0:          #self.sample_rate * 1
...
class SpeechToTextTranscriber:
    ...
    def transcribe_real_time(self, audio_data):
        ...
        try:
            ...
            if detected_language_list:
                logging.info(f"Audio language detection: {detected_language_list}")
    ...
    def detect_language_from_audio(self, audio_data):
        ...
        try:
            ...
            # Classify the language using VoxLingua
            prediction = self.audio_language_model.classify_batch(audio_input)
            voxlingua_language = prediction[3]  # Top prediction
...
if __name__ == "__main__":
    ...
    # Let the recording run until user stops it
